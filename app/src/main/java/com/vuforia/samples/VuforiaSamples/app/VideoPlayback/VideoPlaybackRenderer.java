...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl {
    ...
    Vec3F targetPositiveDimensions[] = new Vec3F[2];
 ...
 /*   // Called when the surface is created or recreated.
    ...
    @SuppressLint("InlinedApi")
    void renderFrame(float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            // calculation
            if (imageTarget.getName().compareTo("keyboard") == 0)
    ...
    boolean isTapOnScreenInsideTarget(int target, float x, float y)
    {
        ...
        // if-statement simply checks that the tap is within this range     //intersection.getdata()[0] returning NAn
        if ((intersection.getData()[0] >= -(targetPositiveDimensions[target]
            .getData()[0])) // return targetPositiveDimensions returns 0.0
